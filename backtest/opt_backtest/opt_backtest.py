        ...
        for day, day_df in day_groups:
            ...
            for idx, row in day_df.iterrows():
                ...
                if is_ce_position:
                    ...
                    if ce_hard_sl and day_df.loc[idx, f"{ce_opt_symbol}_close"] > ce_hard_sl:
                        tradebook["datetime"].append(day_df.loc[idx, "date"])
                        tradebook["price"].append(day_df.loc[idx, f"{ce_opt_symbol}_close"])
                        tradebook["side"].append(1)
                        tradebook["traded_quantity"].append(quantity)
                        self.backtest_logger.logger.info(
                            f"{curr_time} HARD SL HIT : {ce_opt_symbol} px@{day_df.loc[idx, f'{ce_opt_symbol}_close']}, PNL: {running_pnl_rupees * -1}")
                        running_pnl_points += day_df.loc[idx, f"{ce_opt_symbol}_close"] * 1
                        running_pnl_rupees += day_df.loc[idx, f"{ce_opt_symbol}_close"] * 1 * quantity
                        is_ce_position = False
                        ce_hard_sl = None
                        quantity = self.strategy.position_size
                ...
                if is_pe_position:
                    # If candle high breaches Hard SL, exit
                    if pe_hard_sl and day_df.loc[idx, f"{pe_opt_symbol}_close"] > pe_hard_sl:
                        tradebook["datetime"].append(day_df.loc[idx, "date"])
                        tradebook["price"].append(day_df.loc[idx, f'{pe_opt_symbol}_close'])
                        tradebook["side"].append(1)
                        tradebook["traded_quantity"].append(quantity)
                        self.backtest_logger.logger.info(
                            f"{curr_time} HARD SL HIT : {pe_opt_symbol} px@{day_df.loc[idx, f'{pe_opt_symbol}_close']}, PNL: {running_pnl_rupees * -1}")
                        running_pnl_points += day_df.loc[idx, f'{pe_opt_symbol}_close'] * 1
                        running_pnl_rupees += day_df.loc[idx, f'{pe_opt_symbol}_close'] * 1 * quantity
                        is_pe_position = False
                        pe_hard_sl = None
                        quantity = self.strategy.position_size
